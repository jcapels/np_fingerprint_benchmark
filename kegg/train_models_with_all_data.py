...
from deepmol.standardizer._utils import heavy_standardisation
from deepmol.standardizer import CustomStandardizer
from deepmol.pipeline_optimization._deepchem_models_objectives import *
import pandas as pd

standardizer = CustomStandardizer(params=heavy_standardisation)
...
def train_and_save_pipeline(data):
    ...
    pipeline = Pipeline(steps=final_steps, path="kegg_robust_multitask_classifier_all_data")
    pipeline.fit(data)
    pipeline.save()
...
if __name__ == "__main__":
    ...
    dataset = pd.read_csv("kegg_pathways_w_labels_dedup.csv", nrows=2)
    labels = dataset.columns[4:]
    # LOAD THE DATA
    loader = CSVLoader('kegg_pathways_w_labels_dedup.csv', 
