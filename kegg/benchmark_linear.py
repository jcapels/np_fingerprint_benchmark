...
def _optimize(datasets, steps, pipeline_name):
    ...
    os.makedirs(f"kegg_prediction_{pipeline_name}_linear", exist_ok=True)
    ...
    for tuple_datasets in datasets:
        ...
        for train_dataset, validation_dataset, test_dataset in tuple_datasets:
            ...
            po = PipelineOptimization(direction='maximize', study_name=f'kegg_prediction_{pipeline_name}_fold_{fold_idx}_rep{rep}_linear', sampler=optuna.samplers.TPESampler(seed=43),
                                    storage=f'sqlite:///kegg_prediction_linear.db')
            ...
            results.to_csv(f"kegg_prediction_{pipeline_name}_linear/kegg_prediction_{pipeline_name}_linear_fold_{fold_idx}_rep{rep}.csv", index=False)
