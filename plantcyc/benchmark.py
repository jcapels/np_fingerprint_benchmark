...
def _optimize(datasets, steps, pipeline_name):
    ...
    os.makedirs(f"plantcyc_pathway_prediction_{pipeline_name}", exist_ok=True)
    ...
    for tuple_datasets in datasets:
        ...
        for train_dataset, validation_dataset, test_dataset in tuple_datasets:
            ...
            po = PipelineOptimization(direction='maximize', study_name=f'plantcyc_pathway_prediction_{pipeline_name}_fold_{fold_idx}_rep{rep}', sampler=optuna.samplers.TPESampler(seed=43),
                                    storage=f'sqlite:///plantcyc_pathway_prediction.db')
            ...
            results.to_csv(f"plantcyc_pathway_prediction_{pipeline_name}/plantcyc_pathway_prediction_{pipeline_name}_fold_{fold_idx}_rep{rep}.csv", index=False)
...
def optimize_for_attentivefp():
    ...
    # OPTIMIZE THE PIPELINE
    _optimize(datasets, attentivefp_steps, 'attentive_fp_7')
