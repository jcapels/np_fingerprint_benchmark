...
from deepmol.compound_featurization import NPClassifierFP, NeuralNPFP, BiosynfoniKeys, MixedFeaturizer, MHFP, MorganFingerprint
...
from deepmol.pipeline_optimization._sklearn_model_objectives import *
...
def _optimize(datasets, steps, pipeline_name):
    ...
    os.makedirs(f"plantcyc_pathway_prediction_{pipeline_name}_linear", exist_ok=True)
    ...
    for tuple_datasets in datasets:
        ...
        for train_dataset, validation_dataset, test_dataset in tuple_datasets:
            ...
            po = PipelineOptimization(direction='maximize', study_name=f'plantcyc_pathway_prediction_{pipeline_name}_fold_{fold_idx}_rep{rep}_linear', sampler=optuna.samplers.TPESampler(seed=43),
                                    storage=f'sqlite:///plantcyc_pathway_prediction_linear.db')
            ...
            results.to_csv(f"plantcyc_pathway_prediction_{pipeline_name}_linear/plantcyc_pathway_prediction_{pipeline_name}_linear_fold_{fold_idx}_rep{rep}.csv", index=False)
            ...
            df.to_csv(f'{pipeline_name}_linear_results.csv', index=False)
...
def optimize_for_np_classifier_fp():
    ...
    def np_classifier_fp_steps(trial, data):
        ...
        model = ridge_classifier_model(ridge_classifier_kwargs=ridge_classifier_kwargs)
        scaler = StandardScaler()

        final_steps.extend([('featurizer', featurizer),  ('scaler', scaler), ('model', model)])
        return final_steps
    ...
    # OPTIMIZE THE PIPELINE
    _optimize(datasets, np_classifier_fp_steps, 'np_classifier_fp_2')
...
def optimize_for_np_bert():
    ...
    def np_bert_steps(trial, data):
        ...
        final_steps.extend([('scaler', scaler), ('model', model)])
        return final_steps
    ...
    # OPTIMIZE THE PIPELINE
    _optimize(datasets, np_bert_steps, 'np_bert_3')
...
def optimize_biosynfoni_np_classifierfp():
    ...
    # OPTIMIZE THE PIPELINE
    _optimize(datasets, biosynfoni_np_classifier_steps, 'biosynfoni_np_classifier_2')
...
def optimize_for_morganfp():
    ...
    # OPTIMIZE THE PIPELINE
    _optimize(datasets, morganfp_steps, 'morganfp_1')
