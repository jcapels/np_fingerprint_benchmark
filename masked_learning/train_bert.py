...
def train_model(train_dataset, validation_dataset):
    ...
    model.save("data/train_test_split_scaffolds/BERT_small")
...
def train_model_large(train_dataset, validation_dataset):
    ...
    model = BERT(vocab_size=train_dataset.tokenizer.vocab_size,
                ...
                accelerator="gpu", strategy="ddp_find_unused_parameters_true", devices=[1, 2, 3, 4, 5, 6, 7], max_epochs=10, batch_size=16,
                metric = Perplexity(), patience=2).fit(train_dataset, validation_dataset=validation_dataset)

    model.save("data/train_test_split_scaffolds/BERT_large")
    
    print(model.evaluate(validation_dataset))

def evaluate_model(test_dataset):
    from deepmol.models import ModernBERT
    model = ModernBERT.load("data/train_test_split_scaffolds/BERT_small", mode="masked_learning")
...
loader = CSVLoaderForMaskedLM(dataset_path="train_test_split_scaffolds/test_dataset_wo_redundancy.csv",
                           smiles_field='smiles',
                           id_field='ids',
                           mode='auto', vocabulary_path="vocab.txt",
                           masking_probability=0.2)
